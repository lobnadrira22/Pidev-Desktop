...
public class RegisterClientFXMLController implements Initializable {
    ...
    public RegisterClientFXMLController() {
    ...
    @FXML
    private void registerClient(ActionEvent event) throws MalformedURLException, IOException {
        if (email.getText().equals("") || password.getText().equals("") || nom.getText().equals("") || prenom.getText().equals("") || dateN.getText().equals("") || genre.getText().equals("")) {
            ...
            if (!email.getText().contains("@") || !email.getText().contains(".")) {
                ...
                try {
                    ...
                    if (count > 0) {
                        Alert al = new Alert(Alert.AlertType.WARNING);
                        al.setTitle("Inscription Client");
                        al.setContentText("Le Client existe déjà dans la base de données !!");
                        al.show();
                    }

                    // Créer l'URL avec les paramètres nécessaires
                    String url = "http://127.0.0.1:8000/signupClientJson?email="+email.getText()+"&password="+password.getText()+"&nom="+nom.getText()+"&prenom="+prenom.getText()+"&genre="+genre.getText()+"&dateNaissance="+dateN.getText();
                    ...
                    all.setTitle("Inscription Client");
                    all.setContentText("Client ajouté !!");
                    all.show();
